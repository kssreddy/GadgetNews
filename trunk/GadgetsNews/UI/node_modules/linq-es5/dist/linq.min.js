...
d.prototype=b===null?Object.create(b):(__.prototype=b.prototype,new __());};var CSharpEnumerator=(function(){function CSharpEnumerator(sourceIterator){this._iterator=sourceIterator;}
...
_super.call(this,iterator);this._set=_set;this._switch=_switch;this._keySelector=_keySelector;}
...
_super.call(this,null);this._current=_current;this._count=_count;this._increment=_increment;}
...
this._current++;return result;};return Generator;}(IteratorBase));exports.Generator=Generator;var DefaultIfEmpty=(function(_super){__extends(DefaultIfEmpty,_super);function DefaultIfEmpty(sourceIterator,_default){_super.call(this,sourceIterator);this._default=_default;}
...
return result;};return DefaultIfEmpty;}(IteratorBase));exports.DefaultIfEmpty=DefaultIfEmpty;var ChunkBy=(function(_super){__extends(ChunkBy,_super);function ChunkBy(target,keySelect,elementSelector,resultSelector){_super.call(this,target);this.keySelect=keySelect;this.elementSelector=elementSelector;this.resultSelector=resultSelector;this.index=0;};ChunkBy.prototype.next=function(value){var result;do{result=this._iterator.next();if(result.done){if(this.box){result.done=false;result.value=this.box;this.box=undefined;return result;}
else
return this._done;}
...
this.key=newKey;this.box.push(this.elementSelector(result.value));}while(!result.done);return this._done;};return ChunkBy;}(IteratorBase));exports.ChunkBy=ChunkBy;var OfType=(function(_super){__extends(OfType,_super);function OfType(target,obj){_super.call(this,target);this.obj=obj;}
OfType.prototype.next=function(value){var result;do{result=this._iterator.next();}while(!result.done&&!(result.value instanceof this.obj));return result;};return OfType;}(IteratorBase));exports.OfType=OfType;var OfValueType=(function(_super){__extends(OfValueType,_super);function OfValueType(target,obj,typeName){_super.call(this,target,obj);this.typeName=typeName;}
...
_super.call(this,iterator);this._method=_method;this._index=_index;}
return MethodIteratror;}(IteratorBase));exports.MethodIteratror=MethodIteratror;var Distinct=(function(_super){__extends(Distinct,_super);function Distinct(){_super.apply(this,arguments);this._set=new Set();}
...
break;}
this._set.add(key);return result;};return Distinct;}(MethodIteratror));exports.Distinct=Distinct;var Where=(function(_super){__extends(Where,_super);function Where(){_super.apply(this,arguments);}
Where.prototype.next=function(value){var result;do{result=this._iterator.next();}while(!result.done&&!this._method(result.value,this._index++));return result;};return Where;}(MethodIteratror));exports.Where=Where;var Skip=(function(_super){__extends(Skip,_super);function Skip(){_super.apply(this,arguments);this._hasSkipped=false;}
...
this._hasSkipped=true;return result;};return Skip;}(MethodIteratror));exports.Skip=Skip;var Take=(function(_super){__extends(Take,_super);function Take(){_super.apply(this,arguments);}
...
return result;};return Take;}(MethodIteratror));exports.Take=Take;var Zip=(function(_super){__extends(Zip,_super);function Zip(first,_second,func){_super.call(this,first,func);this._second=_second;}
...
return{done:false,value:this._method(first.value,second.value)};};return Zip;}(MethodIteratror));exports.Zip=Zip;var Select=(function(_super){__extends(Select,_super);function Select(){_super.apply(this,arguments);}
...
_super.call(this,sourceIterator,selector);this._collectionState=this._done;this._resultState=this._done;this._resultSelector=transform;}
...
return this._done;this._collection=this._method(this._collectionState.value)[Symbol.iterator]();}
this._resultState=this._collection.next();if(!this._resultState.done){this._resultState.value=this._resultSelector(this._collectionState.value,this._resultState.value);}}while(this._resultState.done);return this._resultState;};return SelectMany;}(MethodIteratror));exports.SelectMany=SelectMany;var Join=(function(_super){__extends(Join,_super);function Join(outer,inner,oKeySelect,iKeySelect,transform){_super.call(this,outer,null);this._method=oKeySelect;var result;this._map=new Map();while(!(result=inner.next()).done){var key=iKeySelect(result.value);var group=this._map.get(key);if('undefined'===typeof group){group=[];this._map.set(key,group);}
...
this._resultState=this._collection.next();if(!this._resultState.done){this._resultState.value=this._resultSelector(this._collectionState.value,this._resultState.value);}}while(this._resultState.done);return this._resultState;};return Join;}(SelectMany));exports.Join=Join;var Union=(function(_super){__extends(Union,_super);function Union(sourceIterator,_keySelector){_super.call(this,sourceIterator,function(o){return o;});this._keySelector=_keySelector;this._set=new Set();}
...
break;}
this._set.add(key);return result;};return Union;}(SelectMany));exports.Union=Union;var GroupBy=(function(_super){__extends(GroupBy,_super);function GroupBy(iterator,resultSelect,_map){_super.call(this,iterator,resultSelect);this._map=_map;}
GroupBy.prototype.next=function(value){var result=this._iterator.next();if(result.done)
return this._done;var iterable=this._map.get(result.value);return{value:this._method(result.value,iterable),done:false};};return GroupBy;}(MethodIteratror));exports.GroupBy=GroupBy;var GroupJoin=(function(_super){__extends(GroupJoin,_super);function GroupJoin(iterator,oKeySelect,_transform,_map){_super.call(this,iterator,oKeySelect);this._transform=_transform;this._map=_map;}
GroupJoin.prototype.next=function(value){var innerSet;var result;do{result=this._iterator.next();if(result.done)
return this._done;var key=this._method(result.value);innerSet=this._map.get(key);}while('undefined'===typeof innerSet);return{value:this._transform(result.value,innerSet),done:false};};return GroupJoin;}(MethodIteratror));exports.GroupJoin=GroupJoin;},{}],2:[function(require,module,exports){"use strict";var __extends=(this&&this.__extends)||function(d,b){for(var p in b)if(b.hasOwnProperty(p))d[p]=b[p];function __(){this.constructor=d;}
d.prototype=b===null?Object.create(b):(__.prototype=b.prototype,new __());};require("es6-shim");var Iterator=require("./iterators");var Constant=require("./utilities");function getEnumerable(TSource){if(TSource===void 0){TSource=null;}
...
EnumerableImpl.prototype[Symbol.iterator]=function(){return(this._factory)?this._factory(this._factoryArg):(null!=this._target)?this._target[Symbol.iterator]():{next:function(){return{done:true,value:undefined};}};};EnumerableImpl.prototype.GetEnumerator=function(){return new Iterator.CSharpEnumerator(this[Symbol.iterator]());};EnumerableImpl.prototype.Aggregate=function(seed,func,resultSelector){if(func===void 0){func=Constant.selfFn;}
if(resultSelector===void 0){resultSelector=Constant.selfFn;}
var zero;var method;var selector;if("function"===typeof seed){method=seed;selector=func;}
...
return new EnumerableImpl(this,function(){return new Iterator.Distinct(_this[Symbol.iterator](),keySelector);});};EnumerableImpl.prototype.Except=function(other,kesSelector){var _this=this;return new EnumerableImpl(this,function(){return new Iterator.Intersect(_this[Symbol.iterator](),Constant.getKeys(other,kesSelector),true,kesSelector);});};EnumerableImpl.prototype.GroupBy=function(selKey,selElement,selResult){if(selElement===void 0){selElement=Constant.selfFn;}
...
return new EnumerableImpl(this,function(){return new Iterator.OfType(_this[Symbol.iterator](),obj);});};EnumerableImpl.prototype.OrderBy=function(keySelect,equal){if(keySelect===void 0){keySelect=Constant.selfFn;}
if(equal===void 0){equal=function(a,b){return a-b;};}
return new OrderedLinq(this,function(array){return new Iterator.ArrayIterator(array,0,function(i){return i>=array.length;});},function(a,b){return equal(keySelect(a),keySelect(b));});};EnumerableImpl.prototype.OrderByDescending=function(keySelect,equal){if(keySelect===void 0){keySelect=Constant.selfFn;}
if(equal===void 0){equal=function(a,b){return a-b;};}
return new OrderedLinq(this,function(array){return new Iterator.ArrayIterator(array,array.length-1,function(i){return 0>i;},-1);},function(a,b){return equal(keySelect(a),keySelect(b));});};EnumerableImpl.prototype.ThenBy=function(keySelect,equal){if(keySelect===void 0){keySelect=Constant.selfFn;}
if(equal===void 0){equal=function(a,b){return a-b;};}
if(this instanceof OrderedLinq){var superEqual=this.equal;this.equal=function(a,b){var result=superEqual(a,b);return(0!=result)?result:equal(keySelect(a),keySelect(b));};return this;}
else{return new OrderedLinq(this,function(array){return new Iterator.ArrayIterator(array,0,function(i){return i>=array.length;});},function(a,b){return equal(keySelect(a),keySelect(b));});}};EnumerableImpl.prototype.ThenByDescending=function(keySelect,equal){if(keySelect===void 0){keySelect=Constant.selfFn;}
if(equal===void 0){equal=function(a,b){return a-b;};}
if(this instanceof OrderedLinq){var superEqual=this.equal;this.equal=function(a,b){var result=superEqual(a,b);return(0!=result)?result:equal(keySelect(a),keySelect(b));};return this;}
else{return new OrderedLinq(this,function(array){return new Iterator.ArrayIterator(array,array.length-1,function(i){return 0>i;},-1);},function(a,b){return equal(keySelect(a),keySelect(b));});}};EnumerableImpl.prototype.Range=function(start,count){return new EnumerableImpl(null,function(){return new Iterator.Generator(start,count,true);});};EnumerableImpl.prototype.Repeat=function(element,count){return new EnumerableImpl(null,function(){return new Iterator.Generator(element,count);});};EnumerableImpl.prototype.Reverse=function(){var array=Array.isArray(this._target)?this._target:this.ToArray();return new EnumerableImpl(null,function(){return new Iterator.ArrayIterator(array,array.length-1,function(i){return 0>i;},-1);});};EnumerableImpl.prototype.Select=function(transform){var _this=this;return new EnumerableImpl(this,function(){return new Iterator.Select(_this[Symbol.iterator](),transform);});};EnumerableImpl.prototype.SelectMany=function(selector,result){var _this=this;if(selector===void 0){selector=Constant.selfFn;}
...
return new Iterator.Where(iterator,Constant.trueFn);});};EnumerableImpl.prototype.SkipWhile=function(predicate){var _this=this;if(predicate===void 0){predicate=function(a,n){return false;};}
...
var aggregate=[this._target,second];return new EnumerableImpl(this,function(){return new Iterator.Union(aggregate[Symbol.iterator](),keySelector);});};EnumerableImpl.prototype.Where=function(predicate){var _this=this;if(predicate===void 0){predicate=Constant.trueFn;}
return new EnumerableImpl(this,function(){return new Iterator.Where(_this[Symbol.iterator](),predicate);});};EnumerableImpl.prototype.Zip=function(second,func){var _this=this;return new EnumerableImpl(this,function(){return new Iterator.Zip(_this[Symbol.iterator](),second[Symbol.iterator](),func);});};return EnumerableImpl;}());var OrderedLinq=(function(_super){__extends(OrderedLinq,_super);function OrderedLinq(target,factory,equal){_super.call(this,target,factory);this.equal=equal;}
OrderedLinq.prototype[Symbol.iterator]=function(){if('undefined'===typeof this._factoryArg){this._factoryArg=this._target.ToArray();this._factoryArg.sort(this.equal);}
return this._factory(this._factoryArg);};return OrderedLinq;}(EnumerableImpl));},{"./iterators":1,"./utilities":3,"es6-shim":5}],3:[function(require,module,exports){"use strict";exports.trueFn=function(){return true;};exports.selfFn=function(o){return o;};exports.defGrouping=function(a,b){if(!b[exports.CONST_KEY]){b[exports.CONST_KEY]=a;}
return b;};function getDefaultVal(type,value){if(value===void 0){value=undefined;}
...
return undefined;}
exports.getDefaultVal=getDefaultVal;function getKeyedMap(iterable,keySelector,selElement){var map=new Map();var result,iterator=iterable[Symbol.iterator]();while(!(result=iterator.next()).done){var key=keySelector(result.value);if(!key)
throw exports.CONST_INVALID_KEY;var group=map.get(key);if(!group){group=[];map.set(key,group);}
group.push(selElement(result.value));}
return map;}
...
group.push(result.value);}
return map;}
exports.getKeyedMapFast=getKeyedMapFast;function getKeys(iterable,keySelector){var set=new Set();var otherIterator=iterable[Symbol.iterator]();var result;if(keySelector){while(!(result=otherIterator.next()).done){set.add(keySelector(result.value));}}
else{while(!(result=otherIterator.next()).done){set.add(result.value);}}
return set;}
...
var Reflect=globals.Reflect;var $String=String;var ES={Call:function Call(F,V){var args=arguments.length>2?arguments[2]:[];if(!ES.IsCallable(F)){throw new TypeError(F+' is not a function');}
return _apply(F,V,args);},RequireObjectCoercible:function(x,optMessage){if(x==null){throw new TypeError(optMessage||'Cannot call method on '+x);}
return x;},TypeIsObject:function(x){if(x===void 0||x===null||x===true||x===false){return false;}
return typeof x==='function'||typeof x==='object';},ToObject:function(o,optMessage){return Object(ES.RequireObjectCoercible(o,optMessage));},IsCallable:isCallable,IsConstructor:function(x){return ES.IsCallable(x);},ToInt32:function(x){return ES.ToNumber(x)>>0;},ToUint32:function(x){return ES.ToNumber(x)>>>0;},ToNumber:function(value){if(_toString(value)==='[object Symbol]'){throw new TypeError('Cannot convert a Symbol value to a number');}
...
return it;},GetMethod:function(o,p){var func=ES.ToObject(o)[p];if(func===void 0||func===null){return void 0;}
...
var S=C[symbolSpecies];if(S===void 0||S===null){return defaultConstructor;}
...
var maxSafeInteger=Math.pow(2,53)-1;defineProperties(Number,{MAX_SAFE_INTEGER:maxSafeInteger,MIN_SAFE_INTEGER:-maxSafeInteger,EPSILON:2.220446049250313e-16,parseInt:globals.parseInt,parseFloat:globals.parseFloat,isFinite:numberIsFinite,isInteger:function isInteger(value){return numberIsFinite(value)&&ES.ToInteger(value)===value;},isSafeInteger:function isSafeInteger(value){return Number.isInteger(value)&&_abs(value)<=Number.MAX_SAFE_INTEGER;},isNaN:numberIsNaN});defineProperty(Number,'parseInt',globals.parseInt,Number.parseInt!==globals.parseInt);if(![,1].find(function(item,idx){return idx===0;})){overrideNative(Array.prototype,'find',ArrayPrototypeShims.find);}
if([,1].findIndex(function(item,idx){return idx===0;})!==0){overrideNative(Array.prototype,'findIndex',ArrayPrototypeShims.findIndex);}
...
var testOrder=function(a){var b=keys(_reduce(a,function(o,k){o[k]=true;return o;},{}));return a.join(':')===b.join(':');};var preservesInsertionOrder=testOrder(['z','a','bb']);var preservesNumericInsertionOrder=testOrder(['z',1,'a','3',2]);if(supportsDescriptors){var fastkey=function fastkey(key){if(!preservesInsertionOrder){return null;}
if(typeof key==='undefined'||key===null){return'^'+ES.ToString(key);}else if(typeof key==='string'){return'$'+key;}else if(typeof key==='number'){if(!preservesNumericInsertionOrder){return'n'+key;}
...
map.set(entry[0],entry[1]);});}else if(iterable instanceof MapConstructor){_call(MapConstructor.prototype.forEach,iterable,function(value,key){map.set(key,value);});}else{var iter,adder;if(iterable!==null&&typeof iterable!=='undefined'){adder=map.set;if(!ES.IsCallable(adder)){throw new TypeError('bad map');}
...
var map=emulateES6construct(this,Map,Map$prototype,{_es6map:true,_head:null,_storage:emptyObject(),_size:0});var head=new MapEntry(null,null);head.next=head.prev=head;map._head=head;if(arguments.length>0){addIterableToMap(Map,map,arguments[0]);}
...
return this._size;});defineProperties(Map$prototype,{get:function get(key){requireMapSlot(this,'get');var fkey=fastkey(key);if(fkey!==null){var entry=this._storage[fkey];if(entry){return entry.value;}else{return;}}
var head=this._head;var i=head;while((i=i.next)!==head){if(ES.SameValueZero(i.key,key)){return i.value;}}},has:function has(key){requireMapSlot(this,'has');var fkey=fastkey(key);if(fkey!==null){return typeof this._storage[fkey]!=='undefined';}
var head=this._head;var i=head;while((i=i.next)!==head){if(ES.SameValueZero(i.key,key)){return true;}}
return false;},set:function set(key,value){requireMapSlot(this,'set');var head=this._head;var i=head;var entry;var fkey=fastkey(key);if(fkey!==null){if(typeof this._storage[fkey]!=='undefined'){this._storage[fkey].value=value;return this;}else{entry=this._storage[fkey]=new MapEntry(key,value);i=head.prev;}}
...
entry.next=this._head;entry.prev=this._head.prev;entry.prev.next=entry;entry.next.prev=entry;this._size+=1;return this;},'delete':function(key){requireMapSlot(this,'delete');var head=this._head;var i=head;var fkey=fastkey(key);if(fkey!==null){if(typeof this._storage[fkey]==='undefined'){return false;}
i=this._storage[fkey].prev;delete this._storage[fkey];}
while((i=i.next)!==head){if(ES.SameValueZero(i.key,key)){i.key=i.value=empty;i.prev.next=i.next;i.next.prev=i.prev;this._size-=1;return true;}}
return false;},clear:function clear(){requireMapSlot(this,'clear');this._size=0;this._storage=emptyObject();var head=this._head;var i=head;var p=i.next;while((i=p)!==head){i.key=i.value=empty;p=i.next;i.next=i.prev=head;}
...
return+k;};var ensureMap=function ensureMap(set){if(!set['[[SetData]]']){var m=set['[[SetData]]']=new collectionShims.Map();_forEach(keys(set._storage),function(key){var k=decodeKey(key);m.set(k,k);});set['[[SetData]]']=m;}
...
if(this['[[SetData]]']){this['[[SetData]]'].clear();}},values:function values(){requireSetSlot(this,'values');ensureMap(this);return this['[[SetData]]'].values();},entries:function entries(){requireSetSlot(this,'entries');ensureMap(this);return this['[[SetData]]'].entries();},forEach:function forEach(callback){requireSetSlot(this,'forEach');var context=arguments.length>1?arguments[1]:null;var entireSet=this;ensureMap(entireSet);this['[[SetData]]'].forEach(function(value,key){if(context){_call(callback,context,key,key,entireSet);}else{callback(key,key,entireSet);}});}});defineProperty(SetShim.prototype,'keys',SetShim.prototype.values,true);addIterator(SetShim.prototype,SetShim.prototype.values);return SetShim;}())};if(globals.Map||globals.Set){var mapAcceptsArguments=valueOrFalseIfThrows(function(){return new Map([[1,2]]).get(1)===2;});if(!mapAcceptsArguments){var OrigMapNoArgs=globals.Map;globals.Map=function Map(){if(!(this instanceof Map)){throw new TypeError('Constructor Map requires "new"');}
var m=new OrigMapNoArgs();if(arguments.length>0){addIterableToMap(Map,m,arguments[0]);}
delete m.constructor;Object.setPrototypeOf(m,globals.Map.prototype);return m;};globals.Map.prototype=create(OrigMapNoArgs.prototype);defineProperty(globals.Map.prototype,'constructor',globals.Map,true);Value.preserveToString(globals.Map,OrigMapNoArgs);}
var testMap=new Map();var mapUsesSameValueZero=(function(){var m=new Map([[1,0],[2,0],[3,0],[4,0]]);m.set(-0,m);return m.get(0)===m&&m.get(-0)===m&&m.has(0)&&m.has(-0);}());var mapSupportsChaining=testMap.set(1,2)===testMap;if(!mapUsesSameValueZero||!mapSupportsChaining){var origMapSet=Map.prototype.set;overrideNative(Map.prototype,'set',function set(k,v){_call(origMapSet,this,k===0?0:k,v);return this;});}
if(!mapUsesSameValueZero){var origMapGet=Map.prototype.get;var origMapHas=Map.prototype.has;defineProperties(Map.prototype,{get:function get(k){return _call(origMapGet,this,k===0?0:k);},has:function has(k){return _call(origMapHas,this,k===0?0:k);}},true);Value.preserveToString(Map.prototype.get,origMapGet);Value.preserveToString(Map.prototype.has,origMapHas);}
...
return globals;}));}).call(this,require('_process'),typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{})},{"_process":4}]},{},[2])(2)});